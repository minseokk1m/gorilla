...
TEN2 = ("the list under the heading '10대 원칙' with FIVE short numbered Korean lines exactly: '6 영주·기사는 가볍게' / '7 고릴라 못 될 종목은 즉시 판다' / '8 뺀 돈은 남은 후보에 재투자' / '9 고릴라 충돌은 결판까지 보유' / '10 대부분의 뉴스는 무시한다' (the tenth line circled in red) and a small label '매도와 잡음'")
...
PAGES = {
 "p01":f"A cinematic episode-title page, {INK} The ENTIRE page is ONE single illustration with NO panel borders and NO second scene: a close view of a plain kraft-cover notebook lying on a wooden desk under a warm lamp, filling the upper two thirds of the page, with a wall calendar behind it showing exactly four dates circled in red; NO people, NO whiteboard, NO study room, NO readable text anywhere in the illustration. The bottom third of the page is a clean dark band containing ONLY the title block below; nothing is drawn beneath the band.",
 ...
 "p13":f"Two panels. (1 ~55%) JU BON-JIL tapping line 10 with emphasis beside the whiteboard which shows {TEN2}; the group attentive. (2 ~45%) close on JU BON-JIL's calm face.",
 ...
 "p15":"Two panels. (1 ~55%) JU BON-JIL explaining beside the whiteboard which shows ONLY a small funnel sketch with a pile of dots entering and one dot leaving (no words); NA BAE-UM nodding. (2 ~45%) the notebook close-up: ONLY these hand-written lines: '뉴스는 10대 원칙 중 하나를 건드릴 때만' / '나머지는 소음'.",
 ...
 "p18":"One panel (~100%). A close-up of the OPEN kraft hunting-log NOTEBOOK on the wooden desk, exactly like the previous page (NO whiteboard, NO people, NO room): the page shows a hand-drawn header '전기차' with a ticked checkbox and four short hand-written lines exactly: '① 캐즘' / '② 골' / '③ 캐즘 위치' / '④ 신호 없음 → 바구니 관찰'; nothing else written.",
 ...
 "p20":"Two panels. (1 ~55%) NA BAE-UM closing the kraft log with a satisfied breath; JU BON-JIL nodding; the whiteboard behind is completely BLANK. (2 ~45%) JU BON-JIL's face turning serious, a 'but' coming.",
 "p21":"Two panels. (1 ~55%) JU BON-JIL asking a hard question, leaning on the table; NA BAE-UM caught off guard; the whiteboard behind is completely BLANK. (2 ~45%) HAN TANG-SU looking away guiltily, cap pulled down.",
 "p22":f"One panel (~100%). The whiteboard shows {FAIL4}; JU BON-JIL to the side; ONLY these four labels.",
 "p23":"Two panels, the four failures as HAN TANG-SU vignettes (each panel split into two small inset scenes, NO readable text in any inset). (1 ~50%) HAN TANG-SU asleep over a kraft notebook; HAN TANG-SU grinning at his phone whose screen shows ONLY blurred thumbs-up shapes. (2 ~50%) HAN TANG-SU staring at a single glowing screen in a dark room; HAN TANG-SU tapping his phone while, behind him, a whiteboard shows ONLY a hand-drawn card with the heading letters 'EXIT' and three blank lines.",
 "p24":f"Two panels. (1 ~55%) JU BON-JIL summing up beside the whiteboard which shows {FAIL4}; NA BAE-UM sober. (2 ~45%) close-up of NA BAE-UM ONLY (late-30s, plain black hair, NO glasses, charcoal shirt); JU BON-JIL must NOT appear; the thought balloon belongs to NA BAE-UM.",
 "p25":f"Two panels, storybook inset ({INK}soft vintage tone, ONLY anonymous ancient-era people, NO readable text). (1 ~55%) the old bearded ORACLE by a dying fire drawing four beasts on a cave wall with a charred stick: a hawk, a camel, an ox, an elephant; MAX and his wife watching. (2 ~45%) the oracle drawing a single tiny harness trying to yoke all four beasts at once, the beasts pulling in four directions, comic.",
 ...
 "p27":"Two panels. (1 ~55%) JU BON-JIL explaining beside the whiteboard which shows ONLY the four beast icons (a hawk, a camel, an ox, an elephant) with their labels '정찰', '완비 분석', '참조사례 확인', '운영·기록'; the TALC five at the table each unconsciously matching a beast, comic. (2 ~45%) NA BAE-UM's notebook close-up: ONLY these hand-written lines: '루틴은 혼자 못 지킨다' / '역할을 나눈다' / '그래서 클럽'.",
 "p28":"Two panels. (1 ~60%, over-the-shoulder) NA BAE-UM writing the summary; JU BON-JIL in the soft background; the whiteboard behind is completely BLANK (no writing of any kind). (2 ~40%) the notebook close-up: ONLY these hand-written lines: '분기에 한 번, 네 칸' / '뉴스는 열 원칙의 체로' / '혼자선 새고, 같이면 버틴다'.",
 "p29":f"Two panels. (1 ~55%) at the door JU BON-JIL turns back with the final hook of the season; the whiteboard behind is completely BLANK. (2 ~45%) a faint dreamy image {INK} a round table with empty chairs and one kraft notebook at each seat; NO text.",
 "p30":f"One panel (~100%). Closing chapter-end, {INK}the round table in the distance, chairs waiting; NA BAE-UM and JU BON-JIL small figures walking toward it; the top of the page has NO band and NO text box; a solid BLACK caption band runs across the very BOTTOM edge of the page (bottom 10%), and THE ONLY TEXT ON THIS ENTIRE PAGE is the specified caption lettered in white INSIDE that bottom black band, centered; do NOT invent any other sentence anywhere.",
}
