...
# ※ 2026-09-13 민석님 확정: 실제(IRL) 투자클럽은 언급하지 않는다. 클럽의 필요성과 만드는 법만 보여주고 시즌 3 예고로 마무리
...
G6X = ("a clean hand-drawn SIX-GRADE table: two columns headed '영장류 정글' (left) and '킹덤 정글' (right); three ROW LABELS written down the far left of the table, one per row, exactly '고릴라 / 영주', '침팬지 / 기사', '원숭이 / 소작농'; the six cells (3 rows x 2 columns) contain NO printed text at all (only pinned paper cards where specified); to the right of the table a separate tall box labeled '위치 라벨' at its top, which contains NO printed text and ONLY the cards specified")
G6 = ("the six-grade table with headers '영장류 정글' and '킹덤 정글' and rows '고릴라 / 영주', '침팬지 / 기사', '원숭이 / 소작농', plus a side box labeled '위치 라벨'")
LOGFORM = ("a clean hand-drawn LOG FORM titled '사냥 일지 양식' with these fields exactly, each as a short line with an empty box after it: '종목·카테고리' / '① 채택 위치' / '② 기대 위치' / '③ 등급' / '④ 신호' / '스탠스' / '반론' / '다음 점검일'")
ROLES = ("five small icons in a row with ONE label each: a hawk labeled '정찰', a camel labeled '완비 분석', an ox labeled '참조사례', an elephant labeled '기록·회계', and a small devil-horned mask labeled '반론'; above them the label '③ 역할 다섯'")
AGENDA = ("a hand-drawn meeting agenda card labeled '④ 월례 안건' with four short lines exactly: '교육 20분' / '후보 발표' / '판별표로 검증' / '반론과 기록'; a small clock icon")
...
PAGES = {
 "p01":f"A cinematic episode-title page, {INK} A warm round table seen from above with eight chairs, a kraft notebook at each seat, one candle-like lamp at center; no readable text. A clean dark band across the middle for the title text, a thin band at the bottom.",
 ...
 "p05":"Two panels. (1 ~55%) JU BON-JIL explaining beside the whiteboard which shows ONLY the heading '클럽의 다섯 규칙' and the row of stick figures labeled '① 여섯에서 열다섯 명'; the group counting themselves, comic. (2 ~45%) HAN TANG-SU counting on his fingers, lips moving.",
 ...
 "p07":"Two panels. (1 ~55%) JU BON-JIL explaining beside the whiteboard which shows ONLY the heading '클럽의 다섯 규칙' and the scroll sketch labeled '② 회칙을 먼저 쓴다' with three tiny icons (a coin, a calendar, a door), counting three fingers; HAN SIL-SOK nodding approvingly. (2 ~45%) close on HAN SIL-SOK, reassured.",
 "p08":f"One panel (~100%). The whiteboard shows the heading '클럽의 다섯 규칙' and {ROLES}; JU BON-JIL to the side; ONLY these labels.",
 "p09":f"Two panels. (1 ~55%) JU BON-JIL pointing at the devil-mask icon on the whiteboard which shows {ROLES}, grinning; AN MID-EO (flip phone) raising his hand slowly, the group laughing. (2 ~45%) close on AN MID-EO's stern face with the faintest smile.",
 "p10":f"One panel (~100%). The whiteboard shows the heading '클럽의 다섯 규칙' and {AGENDA}; JU BON-JIL to the side; ONLY these labels.",
 "p11":f"Two panels. (1 ~55%) JU BON-JIL explaining beside the whiteboard which shows {AGENDA}; NA BAE-UM copying. (2 ~45%) close on the same card ({AGENDA}) as JU BON-JIL taps the line '반론과 기록' twice.",
 ...
 "p14":f"One panel (~100%). The whiteboard shows {LOGFORM}; JU BON-JIL to the side; ONLY these labels.",
 "p15":f"Two panels. (1 ~55%) JU BON-JIL explaining beside the whiteboard which shows {LOGFORM}; NA BAE-UM nodding. (2 ~45%) close-up of NA BAE-UM ONLY (late-30s, plain black hair, NO glasses, charcoal shirt); JU BON-JIL must NOT appear; the thought balloon belongs to NA BAE-UM.",
 "p16":"Two panels. (1 ~55%) JU BON-JIL clapping once, announcing the first official meeting; the group straightening up; the whiteboard behind is completely BLANK. (2 ~45%) the group rearranging chairs into a circle around the long table, HAN TANG-SU dragging a chair, comic energy.",
 ...
 "p18":f"One panel (~100%). {LOCK_HS}The whiteboard shows {G6X}; every cell and the 위치 라벨 box are completely EMPTY (no cards yet); ONLY the column headers, the three row labels and the box title.",
 "p19":f"One panel (~100%). {LOCK_HS}The whiteboard shows {G6X} with small white paper cards pinned, each bearing ONLY a company name: in the LEFT (영장류) column of the FIRST row (고릴라 / 영주) four cards 'NVIDIA', 'ASML', 'TSMC', 'ARM'; in the LEFT column of the SECOND row (침팬지 / 기사) one card 'AMD'; inside the '위치 라벨' box exactly ONE card 'Tesla' and nothing else; every other cell EMPTY; HAN SIL-SOK pinning the Tesla card; ONLY these labels and these six names.",
 "p20":f"One panel (~100%). {LOCK_HS}The whiteboard shows {G6X} with white paper cards pinned, each bearing ONLY a company name: FIRST row (고릴라 / 영주): LEFT column cards 'NVIDIA', 'ASML', 'TSMC', 'ARM', RIGHT column card 'Oracle', and one card 'SK하이닉스' pinned exactly ON the vertical line between the left and right cells of this row with a small double-headed arrow; SECOND row (침팬지 / 기사): LEFT column card 'AMD', RIGHT column EMPTY; THIRD row (원숭이 / 소작농): LEFT column three cards 'SanDisk', 'WDC', 'Seagate', RIGHT column EMPTY; the '위치 라벨' box contains exactly ONE card 'Tesla'; HAN SIL-SOK explaining; ONLY these labels and these ten names, nothing else on the board.",
 "p21":f"Two panels. (1 ~55%) {LOCK_HS}HAN SIL-SOK explaining beside the whiteboard which shows {G6X} with the SAME ten cards as before (FIRST row: 'NVIDIA', 'ASML', 'TSMC', 'ARM' left, 'Oracle' right, 'SK하이닉스' on the dividing line; SECOND row: 'AMD' left; THIRD row: 'SanDisk', 'WDC', 'Seagate' left; the '위치 라벨' box holds ONLY 'Tesla'); NO other company names anywhere on the board; NA BAE-UM nodding. (2 ~45%) AN MID-EO raising the objection hand, the group turning with mock dread.",
 "p22":"Two panels. (1 ~55%) AN MID-EO (stern, flip phone pointed like a gavel) objecting, his balloon tail pointing at him; HAN SIL-SOK taking it calmly; the whiteboard behind is out of focus with no readable text. (2 ~45%) SHIN JUNG-HAE (cardigan) writing the objection into a log, the recorder role.",
 ...
 "p24":"Two panels. (1 ~55%) HAN SIL-SOK (black hair, glasses, knit vest) voicing the discovery, his balloon tail pointing at him, beside the whiteboard corner which shows ONLY the pie chart with a small slice labeled '확실한 고릴라' and a large slice labeled '나머지'; the group surprised. (2 ~45%) JU BON-JIL's face, a knowing smile, as if he was waiting for this.",
 "p25":"Two panels. (1 ~55%) JU BON-JIL naming it the next season's question, beside the whiteboard corner which shows ONLY the pie chart labeled '확실한 고릴라' / '나머지'; NA BAE-UM writing fast. (2 ~45%) NA BAE-UM's log close-up: ONLY these hand-written lines: 'ETF는 후보를 찾아준다' / '고릴라에 집중하는 건 우리 몫' / '다음 시즌의 질문'.",
 "p26":"Two panels. (1 ~55%) HAN SIL-SOK (black hair, glasses, knit vest) sitting down, then hesitantly raising a hand again with a personal question, his balloon tail pointing at him; the group softening; the whiteboard behind is out of focus with no readable text. (2 ~45%) close on HAN SIL-SOK, sincere.",
 "p27":"Two panels. (1 ~55%) JU BON-JIL walking over to HAN SIL-SOK and putting a hand on his shoulder; NA BAE-UM moved; the whiteboard behind is completely BLANK. (2 ~45%) the whole group turning toward HAN SIL-SOK with warm nods; SHIN JUNG-HAE smiling.",
 "p28":"Two panels. (1 ~55%) JU BON-JIL explaining why this moment matters; NA BAE-UM's eyes shining; the whiteboard behind is completely BLANK. (2 ~45%) close-up of NA BAE-UM ONLY (late-30s, plain black hair, NO glasses, charcoal shirt); JU BON-JIL must NOT appear; the thought balloon belongs to NA BAE-UM.",
 ...
 "p32":"One panel (~100%). The whiteboard shows ONLY the LEARNING ROADMAP: four boxes '① 기술수용주기 이해', '② 사례로 익히기', '③ 투자와 연결', '④ 실전', ALL FOUR carrying big GREEN check marks, brackets '시즌 1' under ①② and '시즌 2' under ③④; the whole club gathered in front of it, JU BON-JIL at the side, NA BAE-UM at center holding his log; warm; ONLY these labels.",
 "p33":f"One panel (~100%). Closing SEASON-2 finale, {INK}the club silhouettes together looking out over a jungle at sunrise, a gorilla silhouette on a far hill with a faint crack of light through it; the top of the page has NO band and NO text box; a solid BLACK caption band runs across the very BOTTOM edge of the page (bottom 10%), and THE ONLY TEXT ON THIS ENTIRE PAGE is the specified caption lettered in white INSIDE that bottom black band, centered; do NOT invent any other sentence anywhere.",
}
